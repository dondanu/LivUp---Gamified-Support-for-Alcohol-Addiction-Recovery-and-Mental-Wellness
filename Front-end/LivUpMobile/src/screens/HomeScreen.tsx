...
type Badge = { id: number; name: string; description: string };
type UserBadge = { id: number; badges?: Badge };
...
export default function HomeScreen() {
  ...
  const loadData = async () => {
    try {
      ...
      if (quotes && quotes.length > 0) {
        const randomQuote = quotes[Math.floor(Math.random() * quotes.length)] as MotivationalQuote;
      ...
      if (profile?.id) {
        ...
        if (badges) setRecentBadges(badges as unknown as UserBadge[]);
        ...
        if (drinkLogs) {
          ...
          for (let i = 0; i < 365; i++) {
            ...
            const log = (drinkLogs as any[]).find((l) => l.date === dateString);
  ...
  const handleSOS = () => {
    // @ts-expect-error simple navigate for placeholder
...
const styles = StyleSheet.create({
  ...
  content: { padding: 16 },
  sosButton: { backgroundColor: '#E74C3C', flexDirection: 'row', alignItems: 'center', justifyContent: 'center', paddingVertical: 16, borderRadius: 12, marginBottom: 20, shadowColor: '#E74C3C', shadowOffset: { width: 0, height: 4 }, shadowOpacity: 0.3, shadowRadius: 8, elevation: 5 },
  sosButtonText: { color: '#FFFFFF', fontSize: 18, fontWeight: 'bold', marginLeft: 8 },
  statsContainer: { flexDirection: 'row', justifyContent: 'space-between', marginBottom: 20 },
  statCard: { flex: 1, marginHorizontal: 4, borderRadius: 16, overflow: 'hidden', shadowColor: '#000', shadowOffset: { width: 0, height: 4 }, shadowOpacity: 0.1, shadowRadius: 8, elevation: 5 },
  ...
  quoteCard: { backgroundColor: '#FFFFFF', padding: 24, borderRadius: 16, marginBottom: 20, shadowColor: '#000', shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.1, shadowRadius: 8, elevation: 3 },
  ...
  levelCard: { borderRadius: 16, overflow: 'hidden', marginBottom: 20, shadowColor: '#000', shadowOffset: { width: 0, height: 4 }, shadowOpacity: 0.1, shadowRadius: 8, elevation: 5 },
  ...
  badgeCard: { flex: 1, backgroundColor: '#FFFFFF', padding: 16, borderRadius: 12, marginHorizontal: 4, alignItems: 'center', shadowColor: '#000', shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.1, shadowRadius: 4, elevation: 3 },
  ...
  actionCard: { flex: 1, marginHorizontal: 4, borderRadius: 12, overflow: 'hidden', shadowColor: '#000', shadowOffset: { width: 0, height: 4 }, shadowOpacity: 0.1, shadowRadius: 8, elevation: 5 },
